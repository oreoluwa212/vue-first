<template>
  <div class="p-6">
    ...
    <!-- <div class="flex justify-between items-center mb-4">
      <span class="text-navy">Page {{ currentPage }} of {{ totalPages }}</span>
      <div>
        <button class="px-4 py-2 bg-gray-200 text-navy rounded mr-2" :disabled="currentPage === 1" @click="prevPage">
          Previous
        </button>
        <button class="px-4 py-2 bg-gray-200 text-navy rounded" :disabled="currentPage === totalPages"
          @click="nextPage">
          Next
        </button>
      </div>
    ...
    <table class="table-auto w-full border shadow-md rounded uppercase">
      ...
      <tbody>
        <tr v-for="item in paginatedData" :key="item.id" @click="toggleRowSelection(item.id)"
